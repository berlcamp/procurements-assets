...
export async function addPpmpLot(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      schedule_quarter: input.schedule_quarter ?? null,
      advertisement_date: input.advertisement_date ?? null,
      bid_opening_date: input.bid_opening_date ?? null,
      award_date: input.award_date ?? null,
      contract_signing_date: input.contract_signing_date ?? null,
