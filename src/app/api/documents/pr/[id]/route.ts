...
import { PurchaseRequestPdf } from "@/lib/pdf/templates/purchase-request"
...
export async function GET(
  ...
  try {
    const element = createElement(PurchaseRequestPdf, { pr, divisionName })
    const pdfBuffer = await renderToBuffer(element as any)

    return new NextResponse(new Uint8Array(pdfBuffer), {
      headers: {
        "Content-Type": "application/pdf",
        "Content-Disposition": `attachment; filename="PR-${pr.pr_number}.pdf"`,
