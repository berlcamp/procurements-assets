-- PPMP → APP amendment propagation logic
--
-- Handles four scenarios when an approved PPMP's items need to flow into the APP:
--
-- 1. First PPMP approval, no APP exists         → create APP + insert items
-- 2. PPMP approval, APP is draft/indicative      → append items
-- 3. PPMP amendment approved, APP still editable → replace old items in-place
-- 4. PPMP amendment approved, APP is final/approved → auto-create supplemental APP version
--
-- Also adds a standalone create_app_amendment() RPC for manual APP amendments.

-- ============================================================
-- 1. Replace the auto_populate trigger to handle amendments
-- ============================================================
...
BEGIN
  ...
  IF v_app_version_id IS NULL AND v_is_amendment THEN
    ...
    -- Clone all existing approved items from the last approved version into the new version
    -- so the supplemental version starts with a complete picture
    ...
    IF v_approved_ver_id IS NOT NULL THEN
      -- Clone items (excluding old items from this same PPMP — they'll be replaced below)
      ...
      SELECT
        ...
        procurement_start, procurement_end, delivery_period,
        budget_allocation_id, source_office_id,
        -- Carry forward HOPE review status for items NOT from this PPMP
        hope_review_status, hope_reviewed_by, hope_reviewed_at, hope_remarks,
        NULL, NULL, remarks, created_by
      FROM procurements.app_items
      WHERE app_version_id = v_approved_ver_id
        AND deleted_at IS NULL
        AND source_ppmp_id <> NEW.id;  -- Exclude items from the amended PPMP

      -- Clone lots (without item assignments — those will be rebuilt)
    ...
    -- Reset APP status to indicate amendment is in progress
  ...
  -- Still no editable version (non-amendment case with locked APP) — bail out
...
-- ============================================================
-- 2. Manual APP amendment RPC
--    Creates a new amendment version cloning the approved version.
-- ============================================================
...
BEGIN
  ...
  -- Clone items (carry forward HOPE review status)
  INSERT INTO procurements.app_items (
    app_version_id, app_id,
    source_ppmp_project_id, source_ppmp_lot_id, source_ppmp_id,
    item_number, general_description, project_type,
  ...
  -- Clone lots (reset to draft so BAC can re-finalize)
  INSERT INTO procurements.app_lots (
    app_id, app_version_id, lot_number, lot_name, description,
    procurement_method, total_estimated_cost, status,
    division_id, created_by
